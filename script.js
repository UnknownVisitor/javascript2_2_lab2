(function() {
  ...
  cw1.addEventListener("click", function() {
    ...
    Promise.all([fetchPromise, delayPromise])
      .then(([posts]) => {
        ...
        let htmlContent = '<h2>Wszystkie posty (z Loading)</h2><ul>';
        ...
        // 2. Usunięcie "Loading..." i wyświetlenie zawartości
        answer.innerHTML = htmlContent;
      })
      .catch(error => {
        console.error('Błąd podczas pobierania danych:', error);
        // W przypadku błędu, "Loading..." znika, zastąpione komunikatem o błędzie.
  ...
  cw2.addEventListener("click", function() {
    const postId = 1; // Post o ID = 1 zgodnie z linkiem w instrukcji
  ...
  cw3.addEventListener("click", function() {
    ...
    Promise.all([postPromise, delay])
      .then(([post]) => {
        // 2. Wyświetlenie komunikatu z ID nowego postu po otrzymaniu odpowiedzi
        answer.innerHTML = `
          <h2>Sukces!</h2>
          <p>Dodano nowy post o <strong>ID = ${post.id}</strong></p>
          <div style="border: 1px dashed green; padding: 10px; margin-top: 15px;">
